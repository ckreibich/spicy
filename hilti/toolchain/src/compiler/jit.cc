...
hilti::rt::filesystem::path save(const CxxCode& code, const hilti::rt::filesystem::path& id, std::size_t hash) {
    const std::size_t cc_hash = code.hash();
...
JIT::JIT(std::shared_ptr<Context> context, bool dump_code)
    : _context(std::move(context)),
      ...
      hash(rt::hashCombine(hilti::configuration().hash(),
...
hilti::Result<std::shared_ptr<const Library>> JIT::_link() {
    ...
    auto lib = hilti::rt::filesystem::temp_directory_path() / util::fmt("__library__%" PRIx64 ".hlto", hash);
    args.push_back("-o");
    args.push_back(lib);
...
void JIT::add(CxxCode d) {
    hash = rt::hashCombine(hash, d.hash());
    _codes.push_back(std::move(d));
}

void JIT::add(const hilti::rt::filesystem::path& p) {
    hash = rt::hashCombine(hash, std::hash<std::string>{}(readFile(p)));
    _files.push_back(p);
}
